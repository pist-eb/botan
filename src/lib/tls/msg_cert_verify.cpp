...
Certificate_Verify::Certificate_Verify(Handshake_IO& io,
   ...
   m_impl(std::make_unique<Certificate_Verify_Impl_12>(io, state, policy, rng, priv_key))
   {
   }
...
*/
Certificate_Verify::Certificate_Verify(const std::vector<uint8_t>& buf) : m_impl(std::make_unique<Certificate_Verify_Impl_12>(buf))
   {
   }
